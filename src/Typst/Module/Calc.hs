...
calcModule =
  M.fromList [
   ...
   , ("floor", makeFunction $ do
         (x :: Double) <- nthArg 1
         pure $ VInteger (floor x))
   , ("fract", makeFunction $ do
         (v :: Val) <- nthArg 1
         case v of
   ...
   , ("round", makeFunction $ do
         (x :: Double) <- nthArg 1
         (digits :: Integer) <- namedArg "digits" <|> pure 0
         pure $
           if digits > 0
              then VFloat $ fromIntegral (round (x * 10^digits) :: Integer)
                            / 10^digits
              else VInteger (round x))
   , ("trunc", makeFunction $ do
         (x :: Double) <- nthArg 1
         pure $ VInteger $ truncate x)
   , ("sqrt", makeFunction $ do
         n <- nthArg 1
         if n < 0
