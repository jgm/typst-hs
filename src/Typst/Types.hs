...
instance Summable Val where
  ...
  maybePlus (VColor c) (VLength l) = -- Stroke '1pt + red'
...
prettyVal expr =
  case expr of
    ...
    VArray xs ->
      P.parens ( P.cat $ P.punctuate ", " $
                 map prettyVal (V.toList xs) )
    VTermItem t d -> prettyVal (VArray [VContent t, VContent d])
    VDict m ->
      P.parens ( P.sep $ P.punctuate ","  $
         (map (\(Identifier k, v) ->
                      text k <> ": " <> prettyVal v) (OM.assocs m)) )
    VDirection d -> text $ T.toLower $ T.pack $ show d
    VFunction _ _ _ -> mempty
    VLabel _ -> mempty
    VCounter _ -> mempty
    VColor (RGB r g b o) ->
      "rgb(" <> text (toPercent r) <> "," <> text (toPercent g) <> "," <>
            text (toPercent b) <> "," <> text (toPercent o) <> ")"
    VColor (CMYK c m y k) ->
      "cmyk(" <> text (toPercent c) <> "," <> text (toPercent m) <> "," <>
            text (toPercent y) <> "," <> text (toPercent k) <> ")"
    ...
    VArguments args ->
      P.parens (
       P.sep (P.punctuate ","
        ...
        [ P.sep (P.punctuate ","
           (map (\(Identifier k, v) ->
               text k <> ": " <> prettyVal v) (OM.assocs (named args))) )
...
prettyContent cs
  ...
  | otherwise =
    P.braces (P.space <>
              P.cat (P.punctuate ", " (map go (F.toList cs))) <> P.space)
 where
   go (Txt t) = "[" <> text t <> "]"
   go (Lab l) = "<" <> text l <> ">"
   go (Elt (Identifier name) _ fields) =
     ...
     (P.cat $ P.punctuate ", "
        (map (\(Identifier k, v) ->
          text k <> ": " <> prettyVal v) (M.toList fields)))
